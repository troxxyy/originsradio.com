...
export const getPaginatedArtists = async (
  ...
  if (!supabase) {
    console.warn('Supabase not configured, returning empty paginated artists');
    return { artists: [], total: 0, pages: 0 };
  }
...
export const getArtistStats = async () => {
  if (!supabase) {
    ...
    return { total: 0, featured: 0, active: 0, inactive: 0, pending: 0 };
  }
  const { data, error } = await supabase
    .from('artists')
    .select('featured');
...
export const getFeaturedArtists = async (): Promise<Artist[]> => {
  if (!supabase) {
    console.warn('Supabase not configured, returning empty featured artists');
    return [];
  }
  const { data, error } = await supabase
    .from('artists')
...
export const getArtistsByLocation = async (location: string): Promise<Artist[]> => {
  if (!supabase) {
    console.warn('Supabase not configured, returning empty artists by location');
    return [];
  }
  const { data, error } = await supabase
    .from('artists')
...
export const getArtistsByGenre = async (genre: string): Promise<Artist[]> => {
  if (!supabase) {
    console.warn('Supabase not configured, returning empty artists by genre');
    return [];
  }
  const { data, error } = await supabase
    .from('artists')
